...
from engine import analyse, analyse_all
...
class ThreadingHTTPServer(ThreadingMixIn, HTTPServer):
    """Handle each request in a separate thread — needed for large file uploads."""
    daemon_threads = True
...
class PleezHandler(BaseHTTPRequestHandler):
    ...
    def send_json(self, data: dict, status: int = 200):
        body = json.dumps(_sanitize(data), indent=2, default=str).encode("utf-8")
    ...
    def do_POST(self):
        ...
        if path not in ("/api/analyse", "/api/analyze", "/api/analyse-all", "/api/analyze-all"):
        ...
        try:
            if path in ("/api/analyse-all", "/api/analyze-all"):
                result = analyse_all(file_bytes)
...
SCHEMA_DOC = {
    ...
    "endpoints": {
        ...
        "POST /api/analyse-all": {
            "description": "Analyse ALL restaurants in an XLSX and return per-restaurant reports + summary table",
            "request": {
                "content_type": "multipart/form-data",
                "field": "file",
                "note": "Same schema as /api/analyse — metrics sheet may contain multiple restaurant_ids"
            },
            "response": {
                "success": True,
                "restaurant_count": "int",
                "restaurant_ids": ["R01", "R02", "..."],
                "summary": [
                    {
                        ...
                        "top_action_roi": "float"
                    }
                ],
                "reports": ["same structure as /api/analyse response per restaurant"]
            },
            "curl_example": (
...
if __name__ == "__main__":
    server = ThreadingHTTPServer((HOST, PORT), PleezHandler)
    print(f"""
╔══════════════════════════════════════════════════════╗
