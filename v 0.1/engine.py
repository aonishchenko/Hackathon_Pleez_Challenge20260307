...
def parse_xlsx(file_bytes: bytes) -> RestaurantProfile:
    """
    Parse a restaurant XLSX file (same schema as hackathon dataset).
    Required sheets: metrics
    Optional sheets: promotions, restaurant_tags, restaurants, orders, promotion_daily_metrics
    """
    ...
    if "metrics" not in sheets:
        raise ValueError("XLSX must contain a 'metrics' sheet with columns: restaurant_id, avg_rating, uptime, total_orders, new_users, returning_users, conversion_rate, rejected_orders, cancelled_orders")

    metrics = xl.parse("metrics")
    _require_cols(metrics, ["restaurant_id", "avg_rating", "uptime", "total_orders"], "metrics")

    restaurant_id = str(metrics["restaurant_id"].iloc[0])
    avg_rating = float(metrics["avg_rating"].dropna().mean()) if "avg_rating" in metrics else 4.0
    avg_uptime = float(metrics["uptime"].dropna().mean()) if "uptime" in metrics else 0.95
    avg_daily_orders = float(metrics["total_orders"].dropna().mean()) if "total_orders" in metrics else 50.0
    avg_conversion = float(metrics["conversion_rate"].dropna().mean()) if "conversion_rate" in metrics.columns else 0.18

    # new user ratio
    if "new_users" in metrics.columns and "returning_users" in metrics.columns:
        total_users = metrics["new_users"].fillna(0) + metrics["returning_users"].fillna(0)
        new_user_ratio = float((metrics["new_users"].fillna(0) / total_users.replace(0, np.nan)).mean())
    else:
        new_user_ratio = 0.2
        notes.append("new_users/returning_users not found — using default 20% new user ratio")

    # rejection rate
    if "rejected_orders" in metrics.columns and "total_orders" in metrics.columns:
        rejection_rate = float((metrics["rejected_orders"].fillna(0) / metrics["total_orders"].replace(0, np.nan)).mean())
    else:
        rejection_rate = 0.02
        notes.append("rejected_orders not found — using default 2% rejection rate")

    # --- restaurant_tags (optional) — filter by restaurant_id ---
    tags = []
    cuisine = "Other"
    if "restaurant_tags" in sheets:
        tags_df = xl.parse("restaurant_tags")
        if "cuisine_tag" in tags_df.columns and "restaurant_id" in tags_df.columns:
            filtered = tags_df[tags_df["restaurant_id"] == restaurant_id]
            tags = list(filtered["cuisine_tag"].dropna().unique())
            if not tags:
                # Fallback: no filter (single-restaurant file)
                tags = list(tags_df["cuisine_tag"].dropna().unique())
            cuisine = _primary_cuisine(tags)
    else:
        notes.append("restaurant_tags sheet not found — cuisine will be 'Other'")

    # --- restaurants (optional) — filter by restaurant_id ---
    avg_food_cost = 30.0
    if "restaurants" in sheets:
        rest_df = xl.parse("restaurants")
        if "restaurant_id" in rest_df.columns:
            rest_row = rest_df[rest_df["restaurant_id"] == restaurant_id]
            if len(rest_row) == 0:
                rest_row = rest_df  # fallback
        else:
            rest_row = rest_df
        for col in ["calculated_avg_food_cost_perc", "estimated_avg_food_cost_perc", "avg_food_cost_perc"]:
            if col in rest_row.columns:
                val = rest_row[col].dropna()
                if len(val) > 0:
                    avg_food_cost = float(val.iloc[0])
                    break

    # --- promotions (optional) — filter by restaurant_id ---
    past_promo_types = []
    best_past_roi = 0.0
    worst_past_roi = 0.0
    avg_past_roi = 0.0
    ...
    if "promotions" in sheets:
        promos = xl.parse("promotions")
        if "restaurant_id" in promos.columns:
            promos = promos[promos["restaurant_id"] == restaurant_id]
        if "promotion_type" in promos.columns:
            past_promo_types = list(promos["promotion_type"].dropna().unique())
        if "roi" in promos.columns:
            roi_vals = promos["roi"].dropna()
            if len(roi_vals) > 0:
...
def analyse(file_bytes: bytes) -> dict:
    ...
    Input:  raw bytes of an XLSX file (same schema as hackathon dataset)
    Output: dict representing AnalysisReport (JSON-serialisable)
    """
    profile, data_notes = parse_xlsx(file_bytes)
    ...
    for rank, (pt, roi) in enumerate(scored[:5], 1):
        is_win = roi > 35.0
        confidence = min(94, max(42, int(65 + (roi - 35) * 0.8))) if is_win else max(38, int(65 - (35 - roi) * 1.2))
